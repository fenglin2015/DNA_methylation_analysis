...
read.table("meth_predict_age_71.txt",header=TRUE) -> meth_age
...
#age_predict*21+20->age_predict_adjust
#as.data.frame(age_predict_adjust)->age_predict
...
write.table(age,"age_info_meth_han.txt",col.names = T,row.names = T,sep="\t",quote = F)
#############################################################################################
...
tiff(filename = "predict_age.tiff",
...
dev.off()
####################################################################################################
meth_order <- norm_beta[,c(1,2)]

order      <- order(abs(meth_order$Coefficient))

meth_order <- meth_order[order,]

norm_beta[,1]  -> meth_loc

norm_beta[,-c(1,2)]  -> norm_beta_adjust

as.matrix(norm_beta_adjust) -> norm_beta_adjust

colnames(norm_beta_adjust) -> FID

rep(FID,each=71)    -> FID

rep(meth_loc,length(FID)/71) -> meth

as.vector(norm_beta_adjust) -> meth_value

data.frame(FID,meth,meth_value) -> meth_info_value

join(meth_info_value,age[,c(1,2,3,6)]) -> meth_info_adjust

na.omit(meth_info_adjust) -> meth_info_adjust

for(i in 1:nrow(meth_info_adjust))
{
  if((meth_info_adjust$meth_age[i]-meth_info_adjust$age) > 20)
  {
    meth_info_adjust$col[i] = "red"
    
  } else {
    meth_info_adjust$col[i] = "black"
  }
...
meth_info_adjust <- within(meth_info_adjust,{meth <- factor(meth, levels = meth_order$meth_name[1:71])})

tiff(filename = "predict_age_71_par.tiff",
     width = 2500, height = 2500, units = "px",res = 300,compression="lzw")

plot1 <-ggplot(meth_info_adjust,aes(x = factor(meth),y = meth_value,color= col,shape = col)) + 
        geom_point(size=1)

plot1 <-plot1 + axisSetting + xlab("") + ylab("")
